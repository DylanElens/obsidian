...
editDistance xs ys = distArray ! (0, 0)
  where
    ...
    distArray = array ((0, 0), (length xs, length ys)) [((i, j), find (i, j)) | i <- [0 .. length xs], j <- [0 .. length ys]]
    find :: (Int, Int) -> Int
    find (i, j)
      | i == length xs = length ys - j
      | j == length ys = length xs - i
      | otherwise = minimum [1 + distArray ! (i, j + 1), 1 + distArray ! (i + 1, j), cost (xs !! i) (ys !! j) + distArray ! (i + 1, j + 1)]
